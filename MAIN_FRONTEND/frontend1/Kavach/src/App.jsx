...
function App() {
  ...
  const ganDetected = latestAudio ? latestAudio.gan_artifact_detected : false;
  const audioScore = latestAudio ? latestAudio.score : 0.12;

  const latestSmsEvent = threatEvents.sms.length > 0 ? threatEvents.sms[0] : null;
  const smsScore = latestSmsEvent ? latestSmsEvent.riskScore : 0.05;
